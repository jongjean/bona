# Good Morning Bona 프로젝트 아키텍처 및 보안 규정 (V0.5)
...
| | | 4.2 `/var/www/bona/data` 등 보안 위배 파일 제거 | 100% | 소스/데이터 찌꺼기 제거 |
| | | 4.3 전체 삭제 후 자동 재배포 무결성 테스트 | 100% | 및 구조화 완료 |
| **Step 5** | **정적 배포 완성** | 5.1 정적 템플릿(Baked Template) 설계 및 제작 | 100% | 데이터 주입형 단일 파일 |
| | | 5.2 배포 엔진 고도화(Data Baking & 날짜 파일 생성) | 100% | YYYY-MM-DD.html 자동생성 |
| | | 5.3 화살기도 데이터 체인(URL Parameter) 로직 탑재 | 100% | 엔진 없는 데이터 상속 |
| | | 5.4 엔진 완전 격리 및 SSR 소스 파기(Caddy/Express) | 100% | 외부 직결 통로 완전 봉쇄 |
| | | 5.5 무결성 실전 검증 (과거/현재 카드 열람 테스트) | 100% | 독립적 서비스 구조 확정 |
